...
import (
	...
	"github.com/thegeeklab/renovate-operator/internal/receiver/github/fixtures"
)
...
var _ = Describe("GitHub Webhook Receiver", func() {
	var (
		Receiver *Receiver
		secret   []byte
	)
	...
	Describe("Validate", func() {
		It("should pass with a valid HMAC signature", func() {
			...
			req.Header.Set("X-Hub-Signature-256", generateValidSignature(secret, body))
		...
		It("should fail if the signature is invalid", func() {
			body := []byte(`{"hello": "world"}`)
			req := httptest.NewRequest(http.MethodPost, "/webhook", bytes.NewReader(body))
			req.Header.Set("X-Hub-Signature-256", "sha256=invalid-hacker-signature")

			err := Receiver.Validate(req, secret, body)
			Expect(err).To(MatchError(ErrInvalidSignature))
		})

		It("should fail if the signature format is wrong", func() {
			body := []byte(`{"hello": "world"}`)
			req := httptest.NewRequest(http.MethodPost, "/webhook", bytes.NewReader(body))
		...
		It("should fail if the signature algorithm is not sha256", func() {
			body := []byte(`{"hello": "world"}`)
			req := httptest.NewRequest(http.MethodPost, "/webhook", bytes.NewReader(body))
			req.Header.Set("X-Hub-Signature-256", "sha1=abc123")
