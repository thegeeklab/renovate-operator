...
var _ = Describe("Renovator Discovery", func() {
	...
	Describe("Annotation Forwarding", func() {
		...
		It("should forward multiple operations in annotation", func() {
			// Create a Renovator instance with multiple operations
			renovator := &renovatev1beta1.Renovator{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-renovator",
					Namespace: "default",
			...
			// Verify the annotation was forwarded with multiple operations
			Expect(discovery.Annotations).NotTo(BeNil())
			Expect(discovery.Annotations).To(HaveKey(renovatev1beta1.RenovatorOperation))
			Expect(discovery.Annotations[renovatev1beta1.RenovatorOperation]).To(Equal(
				renovatev1beta1.OperationDiscover + ";" + "other-operation",
			))
		})
